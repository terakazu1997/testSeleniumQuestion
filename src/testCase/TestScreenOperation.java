...
// 画面動作確認
public class TestScreenOperation extends BaseTestVariable{

    public TestScreenOperation(WebDriver driver, String targetFolderPath,String currentFolderKey,Map<String, List<String>> imgFileListMap) throws IOException {
        super(driver,targetFolderPath,currentFolderKey,imgFileListMap);
        file = new FileWriter("./log/画面動作確認テスト.txt", true);
    ...
    public void execute() throws InterruptedException, IOException {
        ...
        //ダイアログのOKボタンを押下する
        alert.accept();
        testFuncs.makeBrowserScreenShot("回答送信ダイアログOK押下");
        Thread.sleep(THREAD_TIME);
        testFuncs.resultUnderScroll("合否結果画面スクロール", THREAD_TIME);
        userData.selectFMUserInfo();
        testFuncs.idClick(idMAP,"retest", THREAD_TIME);
        pw.close();
    }
}